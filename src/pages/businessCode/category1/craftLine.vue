<template>
  <a-card>
    ...
    <CraftLineModal ref="CraftLineModalRefs" @ok="getPageList"></CraftLineModal>
  </a-card>
</template>
    
<script>
import { getPageList } from "@/services/businessCode/category1/craftLine";
import { checkPermission } from "@/utils/abp";
import { mapGetters } from "vuex";
import CraftLineModal from "./modules/CraftLineModal.vue";
...
const columns = [
  ...
  },
  {
    ...
    scopedSlots: {
      customRender: "processRote"
    }
  },
  {
    title: "插件点数",
    dataIndex: "dipPointNum",
    scopedSlots: {
      customRender: "dipPointNum"
    }
  },
  {
    title: "手焊点数",
    dataIndex: "manualWeldingPointNum",
    scopedSlots: {
      customRender: "manualWeldingPointNum"
    }
  },
  {
    title: "贴片点数",
    dataIndex: "patchPointNum",
    scopedSlots: {
      customRender: "patchPointNum"
    }
  },
  {
    title: "SMT价格",
    dataIndex: "smtPrice",
    scopedSlots: {
      customRender: "smtPrice"
    }
  },
  {
    title: "PCBA价格",
    dataIndex: "pcbaTotalPrice",
    scopedSlots: {
      customRender: "pcbaTotalPrice"
    }
  },
  {
    title: "组装价格",
    dataIndex: "assemblyTotalPrice",
    scopedSlots: {
      customRender: "assemblyTotalPrice"
    }
  },
  {
    title: "加工费总价",
    ...
    scopedSlots: {
      customRender: "productTotalPrice"
    }
  }
];
export default {
  components: { CraftLineModal },
  data() {
    return {
      selectedRowKeys: [],
      queryFrom: {
  ...
  methods: {
    ...
    //新增
    add_pagelist() {
      this.$refs.CraftLineModalRefs.openModules("add");
    },
    //编辑
    essentialData_edit(record) {
      this.$refs.CraftLineModalRefs.openModules("edit", record);
    },
    //获取列表数据
    getPageList() {
      const params = {
