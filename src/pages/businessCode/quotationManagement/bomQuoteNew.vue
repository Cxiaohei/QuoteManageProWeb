<template>
  <a-card>
    ...
    <vxe-column field="action" width="80px" title="操作">
        ...
        <a-popconfirm
          ...
          <a-popconfirm
            ...
            ok-text="确定"
            cancel-text="取消"
            @confirm="reCreateBomQuoteNew(row)"
          >
    ...
    <BomQuoteNewModal ref="BomQuoteNewModalRefs" @ok="getPageList"></BomQuoteNewModal>
    <LogListModal ref="LogListModalRefs"></LogListModal>
  </a-card>
</template>
      
  <script>
import { getPageList,deleteBomQuoteNew,reCreateBomQuoteNew } from "@/services/businessCode/quotationManagement/bomQuoteNew";
import { checkPermission } from "@/utils/abp";
import { mapGetters } from "vuex";
import BomQuoteNewModal from "./modules/BomQuoteNewModal.vue";
...
export default {
  components: { BomQuoteNewModal,LogListModal },
  data() {
    return {
        // 表格配置
    customConfig: {
  ...
  methods: {
    reCreateBomQuoteNew(record){
      reCreateBomQuoteNew(record.id).then(res => {
        if(res.code == 1) {
    ...
    //新增
    add_pagelist() {
      this.$refs.BomQuoteNewModalRefs.openModules("add");
    },
    //编辑
    essentialData_edit(record) {
      this.$refs.BomQuoteNewModalRefs.openModules("edit", record);
    },
    showLog(record) {
    ...
    deleteBomQuoteNew(record){
      deleteBomQuoteNew(record.id).then(res => {
